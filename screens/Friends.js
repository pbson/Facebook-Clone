...
const Friends = ({ navigation }) => {
    ...
    const fetchSuggestedFriends = async () => {
        ...
        const url = `http://192.168.43.210:3000/it4788/user/get_list_suggested_friends?token=${savedToken}&index=${index}&count=${count}`
    ...
    const fetchResquestedFriends = async () => {
        let savedToken = await AsyncStorage.getItem('savedToken');
        const url = `http://192.168.43.210:3000/it4788/user/get_requested_friends?token=${savedToken}&index=${index}&count=${count}`
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
