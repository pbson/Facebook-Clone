...
const Comment = ({ route }) => {
    ...
    const changeData = async (event) => {
        ...
        let index = 0;
        let count = 1;
        const url = `http://192.168.43.210:3000/it4788/comment/set_comment?token=${savedToken}&id=${route.params.id}&index=${index}&count=${count}&comment=${comment}`
    ...
    const fetchResult = async () => {
        let savedToken = await AsyncStorage.getItem('savedToken');
        const url = `http://192.168.43.210:3000/it4788/comment/get_comment?token=${savedToken}&id=${route.params.id}&index=${pageCurrent}&count=10`
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
