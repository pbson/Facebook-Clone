...
const ChatView = ({ route }) => {
    ...
    const [socket] = io('https://project-facebook-clone.herokuapp.com', {jsonp:false, transports: ['websocket'], });

    useEffect(() => {
        //get message
        let url = `https://project-facebook-clone.herokuapp.com/it4788/chatsocket/get_conversation?token=${token}&partner_id=${partnerId}&conversation_id=${conversationId}&index=${index}&count=${count}`
