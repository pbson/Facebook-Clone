...
const AddFriendList = ({ id, addFriendImg, addFriendName, addFriendMutual, isRequest }) => {
    ...
    const addButtonClick = async () => {
        if (isRequest === true) {
            ...
            const url = `http://192.168.43.210:3000/it4788/user/set_accept_friend?token=${savedToken}&user_id=${id}&is_accept=1`
            ...
            let savedToken = await AsyncStorage.getItem('savedToken');
            const url = `http://192.168.43.210:3000/it4788/user/set_request_friend?token=${savedToken}&user_id=${id}`
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
    ...
    const deleteButtonClick = async () => {
        if (isRequest === true) {
            let savedToken = await AsyncStorage.getItem('savedToken');
            const url = `http://192.168.43.210:3000/it4788/user/set_accept_friend?token=${savedToken}&user_id=${id}&is_accept=0`
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
